...
function folderContainsIno(targetPath, depth = 0) {
  ...
  for (const entry of entries) {
    if (entry.isFile() && entry.name.toLowerCase().endsWith(".ino")) return true;
    if (entry.isDirectory()) {
      const dirName = entry.name;
      if (isHiddenDir(dirName) || INTERNAL_IGNORE_DIRS.has(dirName)) continue;
      if (folderContainsIno(path.join(targetPath, dirName), depth + 1)) return true;
    }
  }
  return false;
}
...
function validateSketchPath(relativePath) {
  ...
  if (!fs.existsSync(absolutePath) || !fs.lstatSync(absolutePath).isDirectory()) return null;
...
function runArduinoCompile({ sketchPath, fqbn, outputDir }) {
  return new Promise((resolve) => {
    const args = ["compile", "--fqbn", fqbn, "--output-dir", outputDir, sketchPath];
    const child = spawn("arduino-cli", args, { cwd: sketchPath, env: process.env });
    let stdout = "";
    let stderr = "";
...
async function prepareCompile(relativePath, fqbn) {
  if (!relativePath || !fqbn) return { ok: false, status: 400, error: "Missing path or fqbn" };
  const normalizedFqbn = String(fqbn).trim();
  const resolved = validateSketchPath(relativePath);
  if (!resolved) return { ok: false, status: 400, error: "Invalid sketch path" };
  if (!folderContainsIno(resolved.absolutePath)) return { ok: false, status: 400, error: "Selected folder does not contain .ino files" };
  ...
  try {
    ...
    return { ok: false, status: 500, error: "Unable to prepare build directory" };
  }
  ...
  const compileLog = [compileResult.stdout, compileResult.stderr].filter(Boolean).join("\n").trim();
  if (compileResult.code !== 0) return { ok: false, status: 500, error: "Compile failed", log: compileLog };
  ...
  if (!artifactPath) return { ok: false, status: 500, error: "Compile succeeded but no artifact was found", log: compileLog };
...
app.post("/api/compile", async (req, res) => {
  ...
  if (!compileResult.ok) {
    return res.status(compileResult.status).json({ error: compileResult.error, log: compileResult.log });
  }
