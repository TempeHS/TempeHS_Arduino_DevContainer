class STK500 {
  ...
  async send(data) {
    ...
    const hex = Array.from(uint8).map(b => b.toString(16).padStart(2,'0').toUpperCase()).join(' ');
    this.log(`TX: ${hex}`);
  ...
  async receive(length, timeout = 1000) {
    ...
    // Note: This simple read loop can hang if the device sends nothing.
    // Web Serial read() does not support timeout natively.
    while (offset < length) {
      ...
      try {
        ...
        if (resp[0] === 0x14 && resp[1] === 0x10) {
          ...
          return true;
            ...
            this.log(`Sync failed: Expected 14 10, got ${resp[0].toString(16)} ${resp[1].toString(16)}`);
        }
      } catch (e) {
  ...
  async enterProgMode() {
    ...
    if (resp[0] !== 0x14 || resp[1] !== 0x10)
      throw new Error(`Failed to enter prog mode. Got: ${resp[0].toString(16)} ${resp[1].toString(16)}`);
  }
  ...
  async flashHex(hexString, progressCallback) {
    ...
    try {
      // Reset Pulse is handled by caller usually, but we can do it if we have access to signals
      // Assuming caller did the reset and we are ready to sync.
